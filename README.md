...
**Play.** Click to walk, attack, or take loot. WASD also moves. **I**, **C**, or **Tab** toggles the character sheet (same key closes it). **L** toggles the combat log (left; drag the title, scroll the wheel, click x to close). **M** toggles the map of explored tiles (right; same drag/close). Esc or Q asks to quit (Esc/Y again leaves; N stays). Space uses / talks to a place. Keys **1–5** use the action bar (new heroes start with HP on 1 and mana on 2). Open the sheet and drag a bag item or a known spell onto a slot; right-click a slot to clear it; drag a slot off the bar to unbind. Haven is safe (vendor, stash, camp, gate). Hover trader stock for stats. Floors can be mud (slow), ice (faster), or embers (fire). The Wilds has imps and four dungeon portals. A dungeon floor stays as you left it (mobs, loot, fog) until every monster is dead; the next visit then rolls a new layout. Depth 2+ dungeons spawn a **Warden** in the last room (slam). Death returns to Haven with a gold tax.
